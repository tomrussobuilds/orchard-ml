...
`# pragma: no mutant` annotations. These are **only** applied to
...
<h3>Writing Mutation-Resilient Tests</h3>
...
<h3>Pragma Conventions</h3>

| Annotation | Scope | Usage |
|---|---|---|
| `# pragma: no mutant` | Single line | `logger.info()`, `logger.debug()`, plot formatting |
| `# pragma: no cover` | Single line | Unreachable defensive code |

**Never** apply `# pragma: no mutant` to:
