...
def _build_comment(field_schema: dict[str, Any]) -> str | None:
    ...
    has_lo = lo_val is not None or lo_exc is not None
    has_hi = hi_val is not None or hi_exc is not None
    ...
    if has_lo:
        if lo_val is not None:
            return f"{desc} (\u2265 {lo_val})"
        return f"{desc} (> {lo_exc})"
    if has_hi:
        if hi_val is not None:
            return f"{desc} (\u2264 {hi_val})"
        return f"{desc} (< {hi_exc})"
